...
if (is_single_breadcrumbs_visible() && (is_single() || is_tax() || is_category())) {
  ...
  // 女優タクソノミーページの場合、ホームの次に「女優一覧」を挟む
  if (is_tax() && $cat && !is_wp_error($cat) && isset($cat->taxonomy) && 'actress' === $cat->taxonomy) {
    $actress_list_page = get_page_by_path('actress-list');
    if ($actress_list_page) {
      ++$count;
      echo '<div class="breadcrumb-item" itemscope itemtype="https://schema.org/ListItem" itemprop="itemListElement"><span class="sp"><span class="fa fa-angle-right" aria-hidden="true"></span></span><span class="fa fa-folder fa-fw" aria-hidden="true"></span><a href="'.esc_url(get_permalink($actress_list_page)).'" itemprop="item"><span itemprop="name" class="breadcrumb-caption">女優一覧</span></a><meta itemprop="position" content="'.$count.'" /></div>';
    }
  }
