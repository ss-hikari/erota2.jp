...
if( !function_exists('bzb_header_social_buttons') ){
  ...
  function bzb_header_social_buttons(){
    ...
    if( isset($facebook_page_url) && $facebook_page_url !== '' ){
      $bzb_header_social_buttons .= '<li class="facebook_icon"><a href="' . $facebook_page_url . '" target="_blank" aria-label="facebook"><i class="fa fa-facebook-square"></i></a></li>';
    }
    if( isset($twitter_from_db) && $twitter_from_db !== '' ){
      $bzb_header_social_buttons .= '<li class="twitter_icon"><a target="_blank" href="https://x.com/' . $twitter_from_db . '" aria-label="X"><i class="fa fa-x"></i></a></li>';
    }    
    $bzb_header_social_buttons .= '<li class="feedly_icon"><a target="_blank" href="https://feedly.com/i/subscription/feed/' . $feedly_url . '" aria-label="feedly"><i class="fa fa-rss"></i></a></li>';
    $bzb_header_social_buttons .= '</ul>';
...
EOF;
    ...
    <li class="bzb-twitter">
      <a href="https://x.com/share" class="twitter-share-button" {$twitter_disp_type} data-url="{$page_url}"  data-text="{$post_title}">Tweet</a>
...
EOF;
      ...
      if($show_line_button){
        if($type){
        $line=<<<EOF
        <li class="bzb-line"><a href="https://line.me/R/msg/text/?{$post_title}%0D%0A{$page_url_encode}" target="_blank"><img src="{$line_box_image}" width="36" height="60" alt="LINEで送る" /></a></li>
EOF;
        }else{
        $line=<<<EOF
        <li class="bzb-line"><a href="https://line.me/R/msg/text/?{$post_title}%0D%0A{$page_url_encode}" target="_blank"><img src="{$line_image}" width="82" height="20" alt="LINEで送る" /></a></li>
EOF;
        }
